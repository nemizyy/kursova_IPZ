"""
strategies.py — Патерн Strategy (Стратегія) (GoF).

РОЛЬ ПАТЕРНА В СИСТЕМІ:
Дозволяє динамічно змінювати алгоритми фільтрації списку майна та генерації звітів 
без зміни коду, який використовує ці алгоритми (клієнта).
Кожен алгоритм (наприклад, фільтр за ціною або звіт у форматі CSV) винесено в окремий клас, 
що імплементує спільний інтерфейс. Це забезпечує легке додавання нових фільтрів або форматів звітів (Open/Closed Principle).
